...
const ADMOB_REWARDED_ID =
    "ca-app-pub-7914086624525579/8614955758";
let admobRewardReady = false;
let admobRewardLoading = false;
let admobListenersInitialized = false;
function getAdMob() {
    try {
        if (
            window.Capacitor &&
...
async function initRewardedAd() {
    const AdMob =
        getAdMob();
    if (!AdMob) {
        console.warn(
            "AdMob plugin is not available"
        );
        return false;
    }
    if (
        admobListenersInitialized
    ) {
        return true;
    }
    admobListenersInitialized = true;
    try {
        if (
            ...
            await AdMob.addListener(
                ...
                () => {
                    admobRewardReady = true;
                    admobRewardLoading = false;
                    console.log(
                        "Rewarded AdMob loaded"
                    );
                }
            );
            await AdMob.addListener(
                ...
                error => {
                    admobRewardReady = false;
                    admobRewardLoading = false;
                    console.warn(
                        "Rewarded AdMob failed to load:",
                        error
                    );
                }
            );
        ...
        console.warn(
            "AdMob listener error:",
            error
        );
        admobListenersInitialized = false;
        return false;
    }
}
async function prepareRewardedAd() {
    ...
    if (admobRewardLoading) {
        return false;
    }
    const initialized =
        await initRewardedAd();
    if (!initialized) {
        return false;
    }
    try {
        admobRewardLoading = true;
...
async function showRewardedAd() {
    ...
    try {
        ...
        /*
            ...
            reward только когда пользователь
            реально получил награду.
            Закрытие рекламы само по себе
            revive НЕ активирует.
        */
        if (
            reward &&
        ...
        admobRewardReady = false;
        console.warn(
            "Rewarded AdMob show error:",
            error
        );
        return false;
    } finally {
        /*
            После просмотра загружаем
            следующую рекламу заранее.
        */
        setTimeout(() => {
            prepareRewardedAd();
        }, 500);
...
/*
    Загружаем Rewarded заранее после
    запуска Capacitor.
*/
function initAdMobAfterStartup() {
    setTimeout(() => {
        prepareRewardedAd();
    }, 1200);
}
...
function endGame() {
    ...
    updateMenuUI();
    /*
        Пока игрок находится на Game Over,
        заранее готовим следующую рекламу.
    */
    prepareRewardedAd();
}
/* =========================================================
...
document.getElementById(
    ...
    /*
        ВАЖНО:
        reviveUsed НЕ устанавливаем здесь.
    ...
    */
    ...
    if (!rewarded) {
        ...
        btn.textContent =
            originalText;
        showToast(
            "WATCH THE AD TO REVIVE"
        );
        return;
    }
    /*
        Только здесь игрок действительно
        получил Rewarded-награду.
    */
    reviveUsed = true;
    gameOver = false;
    running = true;
...
showScreen(menu);
/*
    Запускаем подготовку Rewarded-рекламы
    после загрузки игры.
*/
initAdMobAfterStartup();